...
Sometimes you want an algorithm that doesn't wrap a bnlearn, pcalg, or Tetrad
implementation at all, but is just a plain R function you wrote yourself. The
`causalDisco` engine's `CausalDiscoSearch$set_alg()` accepts such a function
directly, so you can reuse its data/knowledge/test handling while swapping out
only the search procedure.
...
`make_method()`, exactly as we did for `hpc()` above:
...
`my_method` can now be used exactly like any built-in method:
