...
- Allow
  [`disco()`](https://bjarkehautop.github.io/causalDisco/reference/disco.md)
  to not take a knowledge object? Currently for no knowledge you have to
  pass `knowledge = knowledge()`. And this is not allowed in some
  algorithms (e.g. `pc` with engine `pcalg`).
