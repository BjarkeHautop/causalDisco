# bnlearnSearch – an R6 front-end for **bnlearn**
...
- `score`:
  ...
  Character scalar naming the score function to pass to **bnlearn**. Can
  be set with `$set_score()`. Recognised values are:
...
- `test`:
  ...
  **bnlearn**. Can be set with set_test(). Recognised values are:
...
- `alg`:
  ...
  algorithm from **bnlearn**. Supply one of the method strings below
  when calling `$set_alg(method)`:
...
Set the prior knowledge for the search algorithm usind a knowledge
object.
